...
The number at the start of each line indicates the *group* the URI
belongs to. You can use any number you like, as long as it’s an integer.
...
The results depend on the ``hdt`` file you’re using. If it includes URIs
from DBPedia, the results look something like this:
...
These URIs are long and difficult to read. Read `the
docs <features.html#using-uri-prefixes>`__
to learn how to make the output look better.

Installation
------------

Dedalov2 requires Python 3 and can be installed using Python’s package
manager, pip.
...
Depending on your use case, you may want to pass the ``-U`` flag or run
